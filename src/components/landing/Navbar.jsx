...
export default function Navbar() {
  ...
  const menuButtonClass = 'bg-white/10 text-white hover:bg-white/20 border border-white/15 backdrop-blur-[4px]'
  ...
  return (
    <>
        ...
        <div className={`mx-auto w-full px-4 sm:px-8 max-w-[1140px] 2xl:max-w-[1600px] 3xl:max-w-[1800px] relative flex items-center justify-between gap-4 transition-all duration-500 ${
          ...
          <div className="flex items-center shrink-0">
            <button 
              ...
              className="bg-transparent px-0 inline-flex h-8 items-center justify-center rounded-full transition-all duration-500 cursor-pointer"
            >
              <img 
                src="/images/prius-logo-white.png" 
                alt="Prius Playa Grande" 
                className={`w-auto object-contain transition-all duration-500 ${
                  isScrolled ? 'h-[28px] md:h-[32px] 2xl:h-[40px]' : 'h-[48px] md:h-[58px] 2xl:h-[68px]'
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0 justify-self-end">
            <a 
              ...
              className={`hidden sm:inline-flex h-8 2xl:h-9 items-center gap-2 rounded-full px-3.5 2xl:px-4.5 text-[10px] 2xl:text-xs font-semibold transition-all duration-500 cursor-pointer ${contactButtonClass}`}
            >
            ...
            <button 
              ...
              className={`flex items-center rounded-full transition-all duration-500 size-8 2xl:size-9 shrink-0 justify-center lg:hidden cursor-pointer ${menuButtonClass}`}
              aria-label="Menu"
            >
            ...
            <button 
              ...
              className="h-8 2xl:h-9 cursor-pointer items-center justify-center whitespace-nowrap rounded-full px-4 2xl:px-6 text-[10px] 2xl:text-xs font-bold hidden sm:inline-flex uppercase tracking-wider transition-all duration-500 bg-gold text-prius-black hover:bg-gold-hover shadow-md font-display"
            >
              RESERVÁ
            </button>
          </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] bg-neutral-950/85 backdrop-blur-[4px] flex flex-col justify-between p-6 sm:p-10 animate-premium-fade lg:hidden overflow-y-auto">
            ...
            {SECTIONS.map((sec) => {
              ...
              return (
                <button
                  key={sec.id}
                  onClick={() => scrollToSection(sec.id)}
                  className="group flex items-center justify-between py-4 px-5 rounded-2xl border border-white/10 bg-neutral-900/90 hover:bg-neutral-800 hover:border-gold/50 transition-all duration-300 text-left cursor-pointer shadow-md"
                >
          ...
          <div className="space-y-4 pt-6 border-t border-white/10 shrink-0">
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-full text-xs font-bold uppercase tracking-wider text-center transition-all cursor-pointer bg-neutral-900 text-white hover:bg-neutral-800 border border-white/15 font-display"
            >
            ...
            <div className="flex items-center justify-between text-[11px] text-white/60 pt-2 font-light">
